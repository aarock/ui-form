...
export function Field({ name, isRequired, render }:FieldProps){
  ...
  return <Controller
    ...
    render={({ field, fieldState, formState }) => <>{render?.({
      ...
      insert: ( items: any[], atIndex?:number ) => field.onChange( field.value?.toSpliced?.( atIndex ?? field.value.length, 0, ...keymap(items) )  || keymap(items)),
      remove: ( atIndex: number ) => field.onChange( field.value?.toSpliced?.(atIndex,1) || [] ),
      set: form.setValue
    }, {
